...
-   iOS Professional 강의 섹션7 Container View Controller
    ...
    -   `UINavigationController`
        ...
        -   ex)유튜브 상단의 아이콘들
        -   두 가지 방식으로 구현
            1)   `Push Pop`: 일반적으로 사용. 두 개의 vc가 `background`와 `foreground`를 push ↔️ pop으로 오가며 전체화면을 장악한다
            2)   `Present and Dismiss`: 기존 vc는 아예 뒤로 물러난다. `modal view controller`형태, dismiss를 해야지만 해당 vc 닫기를 호출한다.
